...
class HyperRegression(nn.Module):
    ...
    def decode(self, hist, nbrs, masks, num_points):
        ...
        target_networks_weights = self.hyper(hist, nbrs, masks)
        if self.logprob_type == "Laplace":
            y = self.sample_laplace((target_networks_weights.size(
                0), num_points, self.input_dim), self.gpu)
        if self.logprob_type == "Normal":
            y = self.sample_gaussian((target_networks_weights.size(
                0), num_points, self.input_dim), None, self.gpu)
...
class highwayNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.soc_embedding_size = (
            ((self.grid_size[0]-4)+1)//2)*self.conv_3x1_depth
    ...
    def forward(self, hist, nbrs, masks):
        ...
        soc_enc = self.soc_maxpool(self.leaky_relu(
            self.conv_3x1(self.leaky_relu(self.soc_conv(soc_enc)))))
