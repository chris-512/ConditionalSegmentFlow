...
n_epochs = 120 * 4
# In case the epochs should be cut short after n iterations
n_its_per_epoch = 32 * 8

weight_decay = 1e-5
betas = (0.9, 0.999)                # concerning adam optimizer

# initialization std. dev. of weights (0.03 is approx xavier)
init_scale = 0.030
# for the first n epochs, lower the lr by a factor of 20
pre_low_lr = 0
...
shuffle_val = False
# use a slice [start:stop] of the entire val set
val_start = 0
val_stop = 5120
...
# load entire architecture (overwrites the prev. 2 options)
load_file = ''
filename = 'output/full_model.pt'   # output filename

checkpoint_save_interval = 60
# Whether to overwrite the old checkpoint with the new one
checkpoint_save_overwrite = False
# Wheter to make a checkpoint with suffix _ABORT if an error occurs
